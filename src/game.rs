...
macro_rules! board {
    ...
    ($pb:expr) => {
        ...
        println!("Your promotion meter: {:.2}%", $pb.adv);
    };
}
...
pub fn fgame(pname: &str) {
    ...
    loop {
        ...
        if cbrd.hours >= 24 {
            cbrd.hours = 0;
            cbrd.day += 1;
            println!("It's a new day! Day: {}", cbrd.day);  // Debugging output for the day transition
        }
        ...
        // After working, update hours and check if a new day begins
        // Print the current hours to debug
        println!("{}", cbrd.hours);

        // Check if hours exceed 24, and reset the hours and increment the day

        // Handle promotion meter
        if cbrd.adv >= 100.0 {
            cbrd.adv = 0.0;
            cbrd.work1 += cbrd.work1 / 2;
            cbrd.work2 += cbrd.work1 / 4;
        }
        ...
        if input.trim() == "save" {
            ...
            cbrd.adv += (cbrd.work1 as f32) / 3.0;
            ...
            cbrd.hours += hours;
            cbrd.adv += (cbrd.work2 as f32) / 3.0;
